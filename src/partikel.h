...
Particle *Particle_New(bool (*deactivatorFunc)(struct Particle *)) {
  Particle *p = (Particle *)PARTIKEL_ALLOC(1, sizeof(Particle));
  if (p == NULL) {
    return NULL;
  }
...
Emitter *Emitter_New(EmitterConfig cfg) {
  Emitter *e = (Emitter *)PARTIKEL_ALLOC(1, sizeof(Emitter));
  if (e == NULL) {
    return NULL;
  }
  e->config = cfg;
  ...
  e->particles = (Particle **)PARTIKEL_ALLOC(e->config.capacity, sizeof(Particle *));
  if (e->particles == NULL) {
    PARTIKEL_FREE(e);
    return NULL;
  }
...
bool Emitter_Reinit(Emitter *e, EmitterConfig cfg) {
  if (cfg.capacity > e->config.capacity) {
    ...
    Particle **newParticles = (Particle **)
...
ParticleSystem *ParticleSystem_New(void) {
  ParticleSystem *ps = (ParticleSystem *)PARTIKEL_ALLOC(1, sizeof(ParticleSystem));
  if (ps == NULL) {
    return NULL;
  }
  ps->active = false;
  ...
  ps->emitters = (Emitter **)PARTIKEL_ALLOC(ps->capacity, sizeof(Emitter *));
  if (ps->emitters == NULL) {
    PARTIKEL_FREE(ps);
    return NULL;
  }
